...
if ctrl_case == 1 
    ...
    T_term = 10 - h_step;  %%仿真时长
end
ctrler_type = 3; %%控制器类型选择标志位，1为校正网络,2为普通单环滑模控制,3为评估变增益控制

ctrler_level = 0 ;  %控制器等级，1-5
Bias_amp = 13;  %拉偏极限设定,10-30%
Bias_case = 16 ; %1-16
 
Uncertn_flag = 1;   %%0为不加入不确定，1为加入不确定
...
Trace_1 =strcat('CtrlData_',num2str(ctrler_level),'_',num2str(Distb_flag),'_',num2str(Uncertn_flag),'.mat');%连接路径和文件名得到完整的文件路径
Trace_2 =strcat('LnrPara_',num2str(ctrler_level),'_',num2str(Distb_flag),'_',num2str(Uncertn_flag),'.mat');%连接路径和文件名得到完整的文件路径
count =0;
Kpout = 3;
PA_Flag = 0;
CP_RankS = 1;
CP_RankD = 1;
GS_flag = 0;  %%增益调度的标准位，保证增益调节周期也为0.5S
%% 仿真初始化
...
%         EulerAng_1 = RK_EulrAng(EulerAng, Omega, h_step); %数值积分求解下一时刻的欧拉角      
        ...
        elseif ctrler_type==3
            [Eror_ang_1, Eror_omg_1, DeltaE_1] = Ctrller_AGSC(CmdAng_1, AeroAng_1, Omega_1,CP_RankS,GS_flag); %%使用校正网络控制器
            [DeltaA_1] = Servor_sys(DeltaE_1,DeltaA); %%将虚拟的控制指令分配到实际舵  %经过舵伺服系统
        else
...
%        CtrlSdt_P(10)= 0;   CtrlSdt_P(11)= 0;     CtrlSdt_P(12)= 0;
        ...
        if ctrler_type==3
             if(rem(Time,0.05)==0) %%系统内环响应时间在0.5s左右，故评估周期设定为0.5s
                [Wndw_Data, PA_Flag] = Window_Slid(CtrlSdt_B, CtrlSdt_P);%%控制性能评估，暂时只做俯仰通道
            end  
            if(PA_Flag==1) %%系统内环响应时间在0.5s左右，故评估周期设定为0.5s
                [CP_RankS,CP_RankD ] = Perfm_Assess(Wndw_Data);%%控制性能评估，暂时只做俯仰通道
                GS_flag = 1;  %%增益调度的标准位，保证增益调节周期也为0.5S
                PA_Flag = 0; %%评估完当前性能，将标志位清零
            else
                GS_flag = 0;  %%增益调度的标准位，保证增益调节周期也为0.5S
                PA_Flag = 0; %%评估完当前性能，将标志位清零                
            end          
...
%          FltData2(step,:) = FltBuf2;
         CtrlBuf = [(Time + h_step), ctrler_level, Bias_amp, Bias_case, Omega_1, CtrlSdt_B, CtrlSdt_P,CP_RankS,CP_RankD,Kpout];%%全部的状态量1+1+1+1+3+6+12=25
