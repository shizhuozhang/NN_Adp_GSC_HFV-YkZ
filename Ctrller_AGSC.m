...
function [Eror_ang, Eror_omg, DeltaE] = Ctrller_AGSC(CmdAng, AeroAng, Omega, Rank,GS_flag)
...
global Rank_lst; %三通道的内环网络上次输出记录值
global Prank_lst; %三通道的内环网络上次输出记录值  

if step==1
    u_lst = [0,0,0]; %初始化参数
    Dlt_lst = [0,0,0]; %初始化参数
    load Kin_para.mat    %内环增益'Kinp','Kiny','Kinr'  插值范围0: 0.01: 100;
    Kinp_x = Kinp;
    Kiny_x = Kiny;
    Kinr_x = Kinr;  
    Rank_lst = 1; %初始认为是1级
...
if GS_flag == 1
    %%%%变增益策略2:对控制性能做闭环反馈调节
    Prank = Rank;
%     Prank = 0.1*Prank_lst + 0.9*Rank_lst;  %%惯性滤波    
    RankBias = 2.5 - Prank;  %期望的性能等级为3级2.3
    if RankBias <= -1       
        dKp = 2.5 * RankBias;
    elseif RankBias > 1
        dKp = 0.8 * RankBias;    
    elseif abs(RankBias)>=0.5
        dKp = 0.3 * RankBias;
    else
        dKp = 0.4 * RankBias;
    end

    %%%%%变增益策略1:根据等级简单地加减增益,认为中间等级3左右较好 
%     Prank = Rank;
%     if Prank ==5
%         dKp = -4;
%     elseif Prank==4
...
q_exp = e_alp*Kpout;  %外环控制器，结果作为内环的输入期望
                        %%%%%%%%%%% 内环角速度控制,使用滞后校正网络 %%%%%%%%%%%
